  ...
  <div>
    ...
    <v-card class="mt-6">
      <v-card-title>
        <div style="width: 100%; display: flex; align-items: center; justify-content: space-between">
          <span style="display: flex; align-items: center">
            <v-icon color="black" left>mdi-format-list-bulleted-square</v-icon>
            Inhaltsverzeichnis ({{ title }})
          </span>
          <span>
            <v-tooltip top>
              <template v-slot:activator="{ on, attrs }">
                <v-btn @click="save" color="primary" v-bind="attrs" v-on="on" small>
                  <v-icon size="medium">mdi-content-save</v-icon>
                </v-btn>
              </template>
              <span>Speichern</span>
            </v-tooltip>
